<template>
  <ion-page>
    <h2 style="text-align:center">Remeras Centennials</h2>
    <ion-content>
      <ion-list v-for="r in remeras" :key="r.id" style="text-align:center">
        <div v-if="r.tipo == 'Remera'">
          {{ r.id }}. {{ r.tipo }} / Talle:{{ r.talle }} / Color:{{ r.color }} / Stock: {{r.stock}} / Imagen:{{ r.imagen }}
          <br>
          <img :src="r.imagen" alt="Imagen del producto" style="max-width: 200px; max-height: 200px;">
          <br>
...
export default {
  ...
  methods: {
    ...
    async cargarLista(){
      try{
        this.remeras = await ropaServices.cargar();
      } catch(e){
        console.log(e);
      }
    },
    ...
    */
    async comprar(id){
      try{
        const remeraAComprar = this.remeras.find(remera => remera.id === id);
        if(remeraAComprar.stock > 0){
          await configServices.comprar(remeraAComprar);
          await this.cargarLista();
        } else {
